...
namespace K9Abp.Application.Authorization
{
    ...
    public class WechatMpAuthorizeFilter : IAsyncAuthorizationFilter, ITransientDependency
    {
        ...
        {
            // 是否在微信浏览器
            var ua = context.HttpContext.Request.Headers[HeaderNames.UserAgent].ToString();
            ...
            if (!context.HttpContext.User.IsAuthenticated())
            {
                ...
                var returnUrl = WebUtility.UrlEncode(context.HttpContext.Request.GetAbsoluteUrl(
                        "Wechat/BaseCallback?returnUrl=" + context.HttpContext.Request.GetEncodedUrl()));
